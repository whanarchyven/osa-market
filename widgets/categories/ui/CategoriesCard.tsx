...
export function CategoriesCard({ category, isActive = false }: CategoriesCardProps) {
    ...
    >
      ...
      <div className="mt-4 space-y-1">
        <div className="flex items-center gap-2">
          <p className="text-base lg:text-lg font-semibold text-foreground transition-colors group-hover:text-primary">
            {category.name}
          </p>
        ...
        {category.showProducts === true && (
          <p className="text-base text-muted-foreground">{category.count} товаров</p>
        )}
      </div>
    </Link>
  )
