...
export function PcAssemblyLeadForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/telegram/assembly', {
        method: 'POST',
  ...
  return (
    <div
      id="zapis-na-sborku"
      className="rounded-2xl border border-primary/30 bg-card/80 p-6 md:p-8"
    >
      <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-3">
        Записаться на консультацию по сборке
      </h3>
      <p className="text-muted-foreground mb-6">
        Оставьте контакт — предложим комплектацию, срок и стоимость под ваш задачи.
      </p>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            rows={4}
            placeholder="Бюджет, игры или софт, хотите ли периферию / монитор"
            className="w-full rounded-lg border border-border bg-background px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none"
          />
        </div>
        <button
